...
- **パネル UI**（アイコンをクリックで開閉）:
  - **概要タブ**: 全媒体の主要メーター一覧、今日/過去30日の推定コスト（API換算額。**円建て表示**・USD併記）、30日トークン合計
  - **媒体別タブ**: 各利用枠のメーターとリセットまでの残り時間、**枯渇予測**（消費ペースから「あと○○で枯渇見込み」を推定）、クレジット消費額、トークンの棒グラフ（Claude、**日別14日 / 時間別24hを切替**）
  - 為替レートは自動取得（12時間キャッシュ）。`config.toml` の `usd_jpy` で固定レートも指定可能
...
usd_jpy = 0                                         # 円換算レート。0で自動取得、正の値で固定
```
...
- 表示されるコストは「同じ使用量を API 従量課金で使った場合」の**換算額（概算）**であり、定額プラン利用時の実際の請求額ではありません。枯渇予測も推定です。
